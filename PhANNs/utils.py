...
def get_train_data(features_dir, model_name, model_number, class_arr, group_arr):
    train_data = []
    for prefix in range(1, 11):
        if prefix == model_number:  # mask out validation data
            continue
        train_data.append(load_data(features_dir, f"{prefix}_{model_name}.p"))

    all_train_data = np.concatenate(train_data, axis=0)
    y = class_arr[(group_arr != model_number) & (group_arr != 11)]

    return all_train_data, y


def get_validation_data(features_dir, model_name, model_number, class_arr, group_arr):
    validation_data = load_data(features_dir, f"{model_number}_{model_name}.p")
    y = class_arr[(group_arr == model_number) & (group_arr != 11)]

    return validation_data, y


def get_test_data(features_dir, model_name, class_arr, group_arr):
    test_data = load_data(features_dir, f"11_{model_name}.p")
